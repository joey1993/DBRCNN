...
def get_datasets_concept5(training_data, target_data):
    data_train = list(open(training_data, "r").readlines())
    data_train = [s.strip().decode('utf-8') for s in data_train]
    data_target = list(open(target_data, "r").readlines())
    
    data_index = [s.replace('\n','').strip().split('\t')[0:2] for s in data_target]
    data_target = [s.replace('\n','').strip().split('\t')[2] for s in data_target]

    datasets = dict()
    datasets['data'] = data_train
    data_cat_set = set()
    data_cat_dic = dict()
    for item in data_target:
        data_cat_set.add(item)
    for i,item in enumerate(list(data_cat_set)):
        if not data_cat_dic.has_key(item):
            data_cat_dic[item] = i
    for i in range(len(data_target)):
        data_target[i] = data_cat_dic[data_target[i]]
    datasets['target'] = data_target
    datasets['index'] = data_index
    datasets['target_names'] = list(data_cat_set)
    return datasets
        ...
        if binary:
            ...
            for line_no in range(vocab_size):
                ...
                while True:
                    ...
                    if ch != b'\n':
                        word.append(ch)
                word = str(b''.join(word).decode(encoding,errors='ignore').encode(encoding,errors='ignore'))
            ...
            for line_no in range(vocab_size):
                ...
                if line == b'':
                    raise EOFError("unexpected end of input; is count incorrect or file otherwise damaged?")
                parts = str(line.replace('\n','').rstrip().decode(encoding,errors='ignore').encode(encoding,errors='ignore')).split(" ")
                if len(parts) != vector_size + 1:
                    raise ValueError("invalid vector on line %s (is this really the text format?)" % (line_no))
                word, vector = parts[0], [float(item) for item in parts[1:]]
